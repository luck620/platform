...
public interface TeacherRepository extends JpaRepository<Teacher,Integer> {
    ...
    List<Teacher> findAllByOthers(String name, String phone, String tno, String mail);

    @Query(nativeQuery = true, value = "SELECT count(t.id) from teacher t " +
            "where case when ?1='' THEN 1=1 ELSE t.name like %?1% END " +
            "AND CASE WHEN ?2='' THEN 1=1 ELSE t.phone like %?2% END " +
            "AND CASE WHEN ?3='' THEN 1=1 ELSE t.tno like %?3% END " +
            "AND CASE WHEN ?4='' THEN 1=1 ELSE t.mail like %?4% END ")
    int findAllByOthersCount(String name, String phone, String tno, String mail);


    @Query(nativeQuery = true, value = "select t.* from teacher t where t.id = ?1")
    Teacher findTeacherById(int id);

    @Query(nativeQuery = true, value = "update teacher t set t.name=?1,t.phone=?2,t.tno=?3,t.mail=?4,t.password=?5,t.image_url=?6 where t.id=?7 ")
    @Modifying
    @Transactional
    int update(String name, String phone, String tno, String mail, String password, String imageUrl, int id);
